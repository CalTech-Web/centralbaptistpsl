    ...
    <section className="bg-green-section py-20 relative overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <Image
          ...
          width={500}
          height={303}
          className="opacity-[0.06] w-[400px] md:w-[500px] h-auto"
          aria-hidden="true"
        />
      </div>
      ...
      <div className="relative max-w-6xl mx-auto px-4">
        <div className="text-center mb-12">
          {isLive ? (
            <>
              <span className="inline-flex items-center gap-2 bg-red-600 text-white text-base font-bold px-4 py-1.5 rounded-full mb-5 animate-pulse">
          ...
          {videoId && (
            <p className={`font-semibold text-white/90 ${isLive ? "text-xl md:text-2xl" : "text-lg"}`}>
              {title}
            </p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 items-center">
          ...
          <div className="lg:col-span-2">
            {isLive ? (
              <p className="text-white text-xl md:text-2xl font-bold leading-relaxed mb-4 font-[family-name:var(--font-playfair)]">
                Join Us Right Now!
              </p>
            ) : (
            ...
            <a
              ...
              className={`inline-flex items-center gap-2 bg-white text-primary px-6 py-3 rounded font-bold hover:bg-light transition-colors duration-200 btn-interact ${!isLive ? "sermon-play-flash" : ""}`}
            >
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0C.488 3.45.029 5.804 0 12c.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0C23.512 20.55 23.971 18.196 24 12c-.029-6.185-.484-8.549-4.385-8.816zM9 16V8l8 4-8 4z" />
              </svg>
              {isLive ? "Watch Live" : "View Past Sermons"}
            </a>
          </div>
        </div>
      </div>
